...
export function Header({ isSignedIn = false }: { isSignedIn?: boolean }) {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 mx-auto w-full max-w-5xl border-b border-transparent md:rounded-xl md:transition-all md:ease-out text-foreground",
        {
          "bg-background/40 backdrop-blur-xl border-border/50 md:top-4 md:max-w-4xl shadow-2xl shadow-black/40":
            scrolled && !open,
